...
\section*{40}
Alice and Bob both know a secret key, a one time pad, which is the same bitlength as the message.
...
The xor operation has the property such that $m \oplus k = c$ and $c \oplus k = m$. Moreover, the xor function is one-to-one, meaning that no other $k$ can derive $c$ from $m$ and vice versa.
