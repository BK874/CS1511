...
\section*{3}
Post's Correspondence Problem can be proved to be undecidable by reduction to $A_{TM}$; however, there are a few details to work out, such as how to guarantee that the solution must start with a given block.

We show that $A_{TM}$ reduces to PCP. The input for $A_{TM}$ is $\langle M, w \rangle$. Assume the solution has to start with the block $\block{\#, \#q_(start)w}$. From here, since the blocks were created in such a way that the solution simulates $M$ on $w$, the solution to PCP will return the configurations of running $M$ on $w$.
$
